...
HardLevel::HardLevel(int width, int height, FASG::ConsoleFontRatios fontRatio) : Level(width, height, fontRatio)
{
}
...
void HardLevel::InitLevel()
{
	...
	timehard.SetLocation(0, 0);
}

void HardLevel::ProcessInputs()
{
	if (_kbhit())
		keyPressed = _getch();
...
void HardLevel::GameLogic()
{

	timehard.CoolDown();

	if (timehard.coolDown < 0)
	{
		gameOver = true;
